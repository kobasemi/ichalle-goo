...
import jp.ac.kansai_u.kutc.firefly.waltzforai.entity.FleshEater;
...
public class GeneManager {
	...
	public void crossEntities(Animal e1, Animal e2){
		...
		// ビルダーの生成
		Animal.Builder b;
		if(e1 instanceof PlantEater){
			b = new PlantEater.PlantEaterBuilder(world, (e1.getX()+e2.getX())/2f, (e1.getY()+e2.getY())/2f, energy);
		}else{
			b = new FleshEater.FleshEaterBuilder(world, (e1.getX()+e2.getX())/2f, (e1.getY()+e2.getY())/2f, energy);
		}
	...
	// 友好エンティティか？
	public boolean isFriend(Animal se, Entity oe){
	...
	public boolean isEdible(Animal se, Entity oe){
		if(se instanceof PlantEater){
			if(oe instanceof Plant){
				return true;
			}
		}else{
			if(oe instanceof PlantEater){
				return true;
			}
		}
		return false;
	...
	// 交雑可能か？
	public boolean canCross(Animal animal, Entity entity) {
		if(animal.getClass().equals(entity.getClass())){
			return true;
		}
		return false;
	}
